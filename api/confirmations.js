...
function createConfirmationRunner(req, res, next) {
    ...
    Document.findOne({ '_id': req.body.id }).populate('confirmedUsers', 'userEmail confirmation')
...
function validateDocumentIdAndConfirm(doc, req) {
    ...
    if (confirmationOrder.length > 0 && confirmationOrder[0] !== req.body.email) throw new Exception.InvalidParameterError(`지금은 해당 사용자의 결재 차례가 아닙니다 ! 다음 결재자는 '${confirmationOrder[0]}' 입니다.`);
    return doc;
};
